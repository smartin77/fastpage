...
/////////////////////////////////////////////////////////////////////////
//Story of born....
//After about two years of using the FastTemplate class (all goods to the creator)
//I get stuck in things that required more complicate processing like nested
//dynamic blocks and not in the last "speed".So after I try to push the FastTemplate has
//far he can, finally I decided to build a new one based on my experience in Php
//(I learned a lot in those two years). 
//So here it is. A small class in a single file (I like this idea,thanks again to FastTemplate)
//and with a clear boost in speed. I let you guys to test and see what I mean.
//One of my goals is that to keep the API as far I can similar to FastTemplate since all my projects
//use that class. This is the reason of function names and the extended FastTemplate class from the
//end of this file. I think you still need some changes in your files in order to replace FastTemplate,
//especially if dynamic blocks have been used.
//To keep things clear, all test I made are regarded to FastTemplate so if someone make some tests
//related to other engines let'me know please.
//Also all comments/bugs/suggestions are welcome.
//That's is.
//////////////////////////////////////////////////////////////////////////
//Some capabilities:
...
///////////////////////////////////////////////////////////////////////////
//modification: 15.03.2011 by Smartin77
//Changes:
//- dynamic block definition in <!-- DBS: _block_name --> and <!-- DBE: _block_name -->
//- stripping of dynamic block with empty string instead of white space
//modification: 6.1.2011 by Smartin77
//- cycle stripping with str_replace on matches because of memory leak with preg_replace
//modification: 15.03.2011 by Martin Mrva
//clear_dynamic
///////////////////////////////////////////////////////////////////////////
class ft
{
...
function define_dynamic($block_name,$parent)
{
    ...
    $block_start="<!-- DBS: $block_name -->";
    $block_end="<!-- DBE: $block_name -->";
...
function parse($out_section,$template_name)
{
    ...
    if($this->strip)
    {
		if (preg_match_all ("/{([A-Z0-9_]+)}/", $t_content, $matches))
		{	// cycle replace on matches instead preg_replace by Shuster
			foreach ($matches as $val)
			{
				$t_content = str_replace($val, '', $t_content);
			}
		}


        // $t_content=ereg_replace("{([A-Z0-9_]+)}",'',$t_content);		// deprecated
		// $t_content=preg_replace("/{([A-Z0-9_]+)}/",'',$t_content);	// memory limit leaks
    }
...
function _parse_dynamic($out_tag,$template_name)
{
    ...
    $block_start="<!-- DBS: $template_name -->";
    $block_end="<!-- DBE: $template_name -->";
...
{	// Martin Mrva 15.03.2011
	...
	*/
    ...
    $m_template=preg_replace("/$block_start(.*)$block_end/sU",'',$m_template);
